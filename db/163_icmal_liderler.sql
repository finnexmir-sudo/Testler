-- =====================================================================
--  163  ICMAL: "En yaxsi sagirdler" (rpc_home -> 'top')
--
--  Muellim panelinde Icmal ekraninda sirali siyahi: ad, qrup, doluluq
--  xetti, faiz.  Melumat yalniz OZ hesabinin sagirdlerindendir.
--
--  Govde 120_icmal_diaqnostikasiz.sql-dendir; yeganə deyisiklik -
--  'recent'-den evvel yeni 'top' acari.  Grant deyismir (authenticated,
--  05_grants.sql).  Anon siyahisina DUSMUR.
-- =====================================================================
  ...
  return jsonb_build_object(
    ...
    'top', coalesce((
      select jsonb_agg(jsonb_build_object(
             ...
             order by avg_pct desc, n desc, full_name)
        from (
          select s.id, s.full_name, s.class_id, c.name class_name,
          ...
          having count(*) >= 3
           order by round(avg(a.percent), 0) desc, count(*) desc
           limit 5
        ) z), '[]'::jsonb),
...
do $$ begin
  raise notice '163 OK - rpc_home artiq "top" (en yaxsi 5 sagird) qaytarir.';
end $$;
