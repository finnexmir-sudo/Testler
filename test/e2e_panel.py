...
with sync_playwright() as pw:
    ...
    print("I2 · Şagirdi arxivə salmaq — yer azad olur")
    ...
    ok(pg.locator("[data-arch]").count() == n0,
       "her aktiv sagirdde «Arxivə sal» var", (pg.locator("[data-arch]").count(), n0))
    ok(pg.locator("details.arxiv").count() == 0, "arxiv bolmesi hele yoxdur")

    ad = pg.locator(".stu b").first.inner_text()
    pg.locator("[data-arch]").first.click()
    pg.wait_for_selector("details.arxiv", timeout=8000)
    ok(pg.locator("details.arxiv").count() == 1, "arxiv bolmesi cixir")
    ok(pg.locator(".stu.off").count() == 1, "sagird arxiv gorunusune kecir",
    ...
    ok(ad in pg.inner_text("details.arxiv"), "arxivde DUZ sagird var", ad)
    ok("Arxivdə" in pg.inner_text(".stu.off"), "sebeb yazilir",
    ...
    ok(pg.locator(".stu.off [data-wa]").count() == 0,
       "arxivde «Göndər» duymesi yoxdur")
    ok(pg.locator(".stu.off .code").count() == 0, "arxivde giris kodu gosterilmir")
    ...
    row = db("select is_active from public.students where full_name = %s",
             (ad,), one=True)
    ok(row is not None and row["is_active"] is False, "bazada arxivlenib",
    ...
    ok("%d şagird" % (n0 - 1) in pg.inner_text("#groups"),
       "qrup setrinde de arxivdeki sayilmir",
       pg.inner_text("#groups").replace("\n", " ")[:70])

    print("I3 · Geri qaytarmaq")
    pg.locator("#groups .item").first.click()
    pg.wait_for_selector("details.arxiv", timeout=8000)
    ...
    pg.wait_for_function(
        ...
        arg=n0, timeout=8000)
    ok(pg.locator(".stu.off").count() == 0, "arxiv bosaldi")
    ok(pg.locator("details.arxiv").count() == 0, "arxiv bolmesi itir")
    row = db("select is_active from public.students where full_name = %s",
             (ad,), one=True)
    ok(row is not None and row["is_active"] is True, "bazada geri qaytarilib",
       row and row["is_active"])
