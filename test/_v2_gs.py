#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Qruplar ekrani + alt zolaq (telefon), sol menyu (masaustu) -> /tmp/claude-0/v2/gs_*.png"""
...
with sync_playwright() as pw:
    ...
    for tag, vp, dpr in (("d", {"width": 1280, "height": 800}, 1), ("m", {"width": 390, "height": 844}, 2)):
        ...
        p.evaluate("location.hash='#/gs'"); p.wait_for_selector("#groups .gcard", timeout=15000); p.wait_for_timeout(800)
        p.screenshot(path="%s/gs_%s.png" % (OUT, tag), full_page=False); print(tag, "gs")
        p.evaluate("location.hash='#/me'"); p.wait_for_selector("#btnMeBank", timeout=15000); p.wait_for_timeout(600)
        p.screenshot(path="%s/me_%s.png" % (OUT, tag), full_page=True); print(tag, "me")
        p.context.close()
    br.close()
print("OK")
