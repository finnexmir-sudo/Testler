#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Qrup ekrani: Movzu menzeresi karti - masaustu + telefon"""
...
with sync_playwright() as pw:
    ...
    for tag, vp, dpr in (("d", {"width": 1280, "height": 800}, 1), ("m", {"width": 390, "height": 844}, 2)):
        ...
        p.wait_for_selector("#gTopics .card.tmap", timeout=20000); p.wait_for_timeout(600)
        p.evaluate("document.querySelector('#gTopics').scrollIntoView({block:'start'})"); p.evaluate("window.scrollBy(0,-70)"); p.wait_for_timeout(300)
        p.screenshot(path="%s/tmap_%s.png" % (OUT, tag)); print(tag)
        p.context.close()
    br.close()
print("OK")
