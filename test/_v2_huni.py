#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""202: e-poct tesdiq zolagi (yeni qeydiyyat) + Idareetmede huni karti"""
import os, time, psycopg2, psycopg2.extras
from playwright.sync_api import sync_playwright
DSN = "host=/tmp port=55432 user=postgres dbname=panel_e2e"; BASE = "http://127.0.0.1:8010/"; PANEL = BASE + "muellim/index.html"
OUT = "/tmp/claude-0/v2"; os.makedirs(OUT, exist_ok=True)
CFG = """window.CFG = { SUPABASE_URL: "http://127.0.0.1:54321", SUPABASE_ANON_KEY: "test-anon-key", STUDENT_URL: "https://bil10.az/sagird/", PARENT_URL: "https://bil10.az/valideyn/", SHOW_PLANS: false };"""
def db(q, one=False):
    with psycopg2.connect(DSN, cursor_factory=psycopg2.extras.RealDictCursor) as c, c.cursor() as cur:
        cur.execute(q)
        if cur.description: return cur.fetchone() if one else cur.fetchall()
HIDE = "#demoBar{display:none!important}"
with sync_playwright() as pw:
    br = pw.chromium.launch(executable_path="/opt/pw-browsers/chromium", args=["--no-sandbox"])
    em = "huni%d@test.az" % int(time.time())
    for tag, vp, dpr in (("d", {"width": 1280, "height": 800}, 1), ("m", {"width": 390, "height": 844}, 2)):
        p = br.new_context(viewport=vp, device_scale_factor=dpr).new_page()
        p.route("**/config.js*", lambda r: r.fulfill(status=200, content_type="application/javascript", body=CFG))
        p.goto(PANEL); p.wait_for_selector("#email", timeout=30000)
        if tag == "d":
            p.click("#btnSwap"); p.wait_for_selector("#fname", timeout=8000)
            p.fill("#fname", "Yeni Müəllim"); p.fill("#email", em); p.fill("#pass", "parol123"); p.click("#btnAuth")
        else:
            p.fill("#email", em); p.fill("#pass", "parol123"); p.click("#btnAuth")
        p.wait_for_selector("#mailBar", timeout=30000); p.wait_for_timeout(600)
        p.screenshot(path="%s/mail_%s.png" % (OUT, tag)); print(tag, "mailbar")
        p.context.close()
    #  Idareetme: demo sahibini admin edib huni kartina baxiriq
    db("delete from public.app_state where key='demo_reset'"); db("select public.rpc_demo_reset()")
    p = br.new_context(viewport={"width": 1280, "height": 800}).new_page()
    p.route("**/config.js*", lambda r: r.fulfill(status=200, content_type="application/javascript", body=CFG))
    p.goto(PANEL + "#/demo"); p.wait_for_selector("#demoBar", timeout=60000); p.wait_for_selector("#groups .gcard", timeout=30000)
    p.add_style_tag(content=HIDE)
    #  #/demo her ziyaretciye NUSXE acir - admin rolu nusxenin sahibine verilir
    a = db("select owner_id from public.accounts where is_demo and id <> app.demo_account() order by created_at desc limit 1", one=True)
    db("insert into public.user_roles (user_id, role) values ('%s', 'admin') on conflict do nothing" % a["owner_id"])
    p.evaluate("location.hash = '#/'"); p.wait_for_timeout(400)
    p.evaluate("location.hash = '#/adm'"); p.wait_for_selector(".card.huni", timeout=30000); p.wait_for_timeout(600)
    p.evaluate("document.querySelector('.card.huni').scrollIntoView({block:'center'})"); p.wait_for_timeout(300)
    p.screenshot(path=OUT + "/huni_d.png"); print("huni")
    p.context.close()
    #  telefon
    a = db("select owner_id from public.accounts where is_demo and id <> app.demo_account() order by created_at desc limit 1", one=True)
    pm = br.new_context(viewport={"width": 390, "height": 844}, device_scale_factor=2).new_page()
    pm.route("**/config.js*", lambda r: r.fulfill(status=200, content_type="application/javascript", body=CFG))
    pm.goto(PANEL + "#/demo"); pm.wait_for_selector("#demoBar", timeout=60000); pm.wait_for_selector("#groups .gcard", timeout=30000)
    pm.add_style_tag(content=HIDE)
    a2 = db("select owner_id from public.accounts where is_demo and id <> app.demo_account() order by created_at desc limit 1", one=True)
    db("insert into public.user_roles (user_id, role) values ('%s', 'admin') on conflict do nothing" % a2["owner_id"])
    pm.evaluate("location.hash = '#/'"); pm.wait_for_timeout(400)
    pm.evaluate("location.hash = '#/adm'"); pm.wait_for_selector(".card.huni", timeout=30000); pm.wait_for_timeout(600)
    pm.evaluate("document.querySelector('.hstuck').scrollIntoView({block:'start'})"); pm.evaluate("window.scrollBy(0,-80)"); pm.wait_for_timeout(300)
    pm.screenshot(path=OUT + "/huni_m.png"); print("huni m")
    pm.context.close(); br.close()
db("delete from public.user_roles where role='admin' and user_id in (select owner_id from public.accounts where is_demo)")
print("OK")
